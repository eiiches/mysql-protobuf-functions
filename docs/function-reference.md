...
#### `pb_message_to_json(descriptor_set_json JSON, full_type_name VARCHAR(512), message LONGBLOB) -> JSON`
...
**Errors:**
- Returns an error if the full_type_name cannot be resolved in the descriptor set
...
SELECT pb_message_to_json(@schema_json, '.com.example.Person', @msg);
```
...
SELECT pb_message_to_json(@schema_json, '.com.example.Person', @msg) AS json_output;
```
