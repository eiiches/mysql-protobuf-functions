...
ALL_LIB_SQL_FILES := $(PROTOBUF_FILES) $(PROTOBUF_JSON_FILES)

ALL_TEST_SQL_FILES := build/protocgenmysql_proto3.pb.sql

ALL_SQL_FILES := $(ALL_LIB_SQL_FILES) $(ALL_TEST_SQL_FILES)

.PHONY: build
build: protobuf.sql protoc-gen-mysql mysql-coverage mysql-ftrace

protobuf.sql: $(ALL_LIB_SQL_FILES)
	set -euo pipefail
	$(RM) $@.tmp
	echo "-- Code generated by common.mk; DO NOT EDIT." > $@.tmp
	$(foreach file,$(ALL_LIB_SQL_FILES),echo >> $@.tmp && cat $(file) >> $@.tmp;)
	mv $@.tmp $@
...
.PHONY: build/protocgenmysql_proto3.pb.sql
build/protocgenmysql_proto3.pb.sql: tests/protocgenmysql_proto3.proto
	protoc --descriptor_set_out=build/protocgenmysql_proto3.binpb \
		--include_imports \
		--proto_path=tests/ \
		$<
	./protoc-gen-mysql \
		--descriptor_set_in=build/protocgenmysql_proto3.binpb \
		--name=protocgenmysql_proto3 \
		--file_naming_strategy=single \
		--prefix_map==pbt_ \
		--generate_methods \
		--mysql_out=build/
...
purge: ensure-test-database
	$(MYSQL_COMMAND) -N -B -e "SELECT CONCAT('DROP ', r.routine_type, ' IF EXISTS ', r.routine_name, ';') AS DROP_STATEMENTS FROM information_schema.routines r WHERE r.routine_schema = database() AND (r.routine_name LIKE 'pb_%' OR r.routine_name LIKE '_pb_%' OR r.routine_name LIKE 'pbt_%');" | $(MYSQL_COMMAND)
